...
const ShoppingCart = () => {
  ...
  return (
    <div className="container mx-auto mt-10">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 text-sm font-semibold text-gray-600">
        ...
        <div key={item.id} className="flex flex-col md:flex-row items-center justify-between border-b border-gray-200 py-4">
          <div className="flex items-center w-full md:w-2/5">
            <img
              ...
              alt={item.title}
              onError={(e) => {
                e.target.src = "https://rukminim1.flixcart.com/image/612/612/krme93k0/lehenga-choli/9/t/e/free-half-sleeve-ghaghra-choli-coconut-original-imag5df5ktsnnypv.jpeg?q=70";
              }}
            />
          ...
          <div className="flex items-center">
              <button className="text-gray-600 mx-2 focus:outline-none">
                <svg className="fill-current w-6 cursor-pointer" viewBox="0 0 448 512">
          ...
          <span className="text-center w-full md:w-1/5">₹{item.discountedPrice}</span>
          <span className="text-center w-full md:w-1/5">₹{item.discountedPrice}</span>
        </div>
      ))}

      <div className="flex justify-center items-center mt-6">
        {/* Quantity modification UI */}
      </div>

      <div className="flex justify-between items-center mt-6">
        <Link to="/" className="text-indigo-600 text-sm flex items-center">
        ...
        <div id="summary" className="w-full md:w-1/4 px-4 py-8 bg-gray-100 rounded-lg">
          ...
          <div>
            <label className="font-medium mb-3 text-sm uppercase">Shipping</label>
            <select className="block p-2 text-gray-600 w-full text-sm border">
              <option>Standard shipping - $10.00</option>
            </select>
          </div>
          <div className="py-6">
            <label htmlFor="promo" className="font-semibold mb-3 text-sm uppercase">Promo Code</label>
            <input type="text" id="promo" placeholder="Enter your code" className="p-2 text-sm w-full border" />
          </div>
          <button className="bg-red-500 hover:bg-red-600 px-6 py-3 text-sm text-white uppercase w-full mb-4 rounded-md">Apply</button>
          <div className="border-t">
            <div className="flex font-semibold justify-between py-4 text-sm uppercase">
              <span>Total cost</span>
              <span>${calculateTotalCost()}</span>
            </div>
            <button className="bg-indigo-500 font-semibold hover:bg-indigo-600 px-6 py-3 text-sm text-white uppercase w-full rounded-md">Checkout</button>
          </div>
        </div>
      </div>
    </div>
